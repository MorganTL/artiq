...
from artiq.gateware.cxp_grabber.stream import (
    CXPCRC32_Checker,
    End_of_line_Marker,
    Frame_Header_Reader,
    Stream_Packet_Arbiter,
)
from artiq.gateware.cxp_grabber.pixel import Pixel_Parser, pixelword_layout

from operator import or_, add

class CXP_Host_Core(Module, AutoCSR):
...
class Stream_Decoder(Module, AutoCSR):
    """
    ...
    def __init__(self, res_width):
        ...
        self.submodules.arbiter = arbiter = cdr(Stream_Packet_Arbiter())
        
        # Buffer to improve timing
        self.submodules.buffer = buffer = cdr(Buffer(word_layout_dchar))

        # CRC 
        self.submodules.crc_checker = crc_checker = cdr(CXPCRC32_Checker())
        ...
        self.submodules.header_reader = header_reader = cdr(Frame_Header_Reader())
