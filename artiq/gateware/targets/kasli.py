...
class GenericMaster(MasterBase):
    def __init__(self, description, hw_rev=None, **kwargs):
        if hw_rev is None:
            hw_rev = description["hw_rev"]
        self.class_name_override = description["variant"]
        has_drtio_over_eem = any(peripheral["type"] in ["shuttler", "phaser_drtio"] for peripheral in description["peripherals"])
...
def main():
    ...
    for peripheral in description["peripherals"]:
        ...
        elif peripheral["type"] == "shuttler" and description["drtio_role"] == "standalone":
            raise ValueError("Shuttler requires DRTIO, please switch role to master")
        elif peripheral["type"] == "phaser_drtio" and description["drtio_role"] == "standalone":
            raise ValueError("Phaser requires DRTIO, please switch role to master")
