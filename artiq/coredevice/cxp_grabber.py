...
class CXP_Grabber:
    ...
    @kernel
    def send_cxp_trigger(self, linktrigger, extra_linktrigger=False):
        """
        Send CoaXpress fixed latency trigger to camera

        :param linktrigger: Send trigger type 0 or 1. In CXP v1.x,
                            linktrigger0 was called `rising edge` and linktrigger1 `falling edge`

        :param triggerdelay: In mu, between 0 to 239 inclusive.
    ...
    def download_local_xml(self, file_path):
        ...
        The file format can be .zip or .xml depending on the camera mode
        .. note:: This is NOT a real time operation

        :param file_path: a relative path on PC
        """
        data = cxp_download_xml_file()
        self.write_xml_file(data, file_path)
